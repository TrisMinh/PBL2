...
void User::AccandNotipopup() {
    ...
    notificationPopup->setFixedSize(250, 300); // Cố định kích thước
    ...
    titleLabel->setFixedHeight(40);  // Đặt chiều cao tiêu đề
...
void User::on_listWidget_clicked(){
    ...
    for (int i = 0; i < ui->SerTable->rowCount(); i++){
        LinkedList<ServiceUsage>::Node* current = ServiceUsage::usageList.begin();
        while (current) {
            qDebug() << "Item clicked: " << ui->listWidget->currentItem()->text();
            if (ui->listWidget->currentItem()->text() == current->data.getRoomID() && ui->SerTable->item(i, 0)->text() == current->data.getServiceID()){
                ui->SerTable->setItem(i, 5, new QTableWidgetItem(QString::fromStdString(current->data.getStatus()? "Active" : "Inactive")));
                QWidget* buttonWidget = new QWidget();
                if (ui->SerTable->item(i, 5)->text() == "Inactive"){
                    ...
                    connect(edit_Ser_btn, &QPushButton::clicked, this, [this, i]() {
                        onedit_Ser_btnClicked(i);
                    });
                    QHBoxLayout* layout = new QHBoxLayout(buttonWidget);
                    ...
                    buttonWidget->setLayout(layout);
                    ui->SerTable->setCellWidget(i, 6, buttonWidget);
                }

                if (ui->SerTable->item(i, 5)->text() == "Active" && ui->SerTable->item(i, 4)->text() == "Tự do"){
                    ...
                    connect(delete_Ser_btn, &QPushButton::clicked, this, [this, i]() {
                        ondelete_Ser_btnClicked(i);
                    });
