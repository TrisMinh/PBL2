...
double Payment::calculateRentMoney(double fullRentAmount, int startDay, int billMonth, int billYear) {
...
void Payment::autocreatePayment() {
    ...
    cout << "0. Tu dong gan 100 cho moi phong (Vi du)\n";
    cout << "1. Tu nhap so luong\n";
    cout << "Nhap lua chon: "; cin >> usageChoice;
    ...
    for (LinkedList<Contract>::Node* current = Contract::contractList.begin(); 
        ...
        if (billMonth == contract.getStartDate().get_month() && 
            ...
            rentAmount = calculateRentMoney(rentAmount,contract.getStartDate().get_day(),billMonth, billYear);
        }
    ...
    if (!hasCreated) { cout << "Khong co bills nao duoc tao! Khong tim thay hop dong phu hop voi thoi gian nay hoac da tao roi." << endl; } 
    else { cout << "Tao bills thanh cong!" << endl; }
}
